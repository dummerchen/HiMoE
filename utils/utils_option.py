...
def get_timestamp():
    return datetime.now().strftime('_%y%module%d_%H%M%S')
...
def parse(opt_path, is_train=True):
    ...
    for phase, dataset in opt['datasets'].items():
        ...
        if 'dataroot_H' in dataset and dataset['dataroot_H'] is not None:
            dataset['dataroot_H'] = [os.path.expanduser(i) for i in dataset['dataroot_H']]
        if 'dataroot_L' in dataset and dataset['dataroot_L'] is not None:
            dataset['dataroot_L'] = [os.path.expanduser(i) if i is not None else i for i in dataset['dataroot_L']]
    ...
    for key, path in opt['path'].items():
        if path and key in opt['path']:
            if type(path) is not list:
                ...
                opt['path'][key] = [os.path.expanduser(i) for i in path]
