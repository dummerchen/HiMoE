...
def texton_filters(n_orient=8, sigma=2):
    """
...
def oe_filters(n_orient, sigma, odd):
    ...
    Each returned filter is an (K+1) x (K+1) matrix where K = 3*sigma and
...
def gaussian_filters(n_orient, sigma, deriv, hlbrt, elongation):
    ...
    Each returned filter is an (K+1) x (K+1) matrix where K = 3*sigma.
    """
...
def textons(m, filters, K):
    ...
    Cluster textons using topk-means clusterer with L2 distance metric
    """
    responses = []
        ...
        for j in range(labels.shape[1]):
            ...
            for o_x in range(offset_min_x, offset_max_x + 1):
                for o_y in range(offset_min_y, offset_max_y):
                    (slice_hist[get_value_by_index(slice_map, index_weights)])[
                        0, get_value_by_index(labels, index_labels)] += get_value_by_index(weights, index_weights)
