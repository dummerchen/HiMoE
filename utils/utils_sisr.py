...
def INVLS_pytorch(FB, FBC, F2B, FR, tau, sf=2):
    ...
    x1 = FB.*FR;
    FBR = BlockMM(nr,nc,Nb,module,x1);
    invW = BlockMM(nr,nc,Nb,module,F2B);
...
def pad_circular(input, padding):
    ...
    :param padding: (tuple): module-elem tuple where module is the degree of convolution
    Returns
...
def BlockMM(nr, nc, Nb, m, x1):
    '''
    myfun = @(block_struct) reshape(block_struct.data,module,1);
    x1 = blockproc(x1,[nr nc],myfun);
    x1 = reshape(x1,module,Nb);
    x1 = sum(x1,2);
    x = reshape(x1,nr,nc);
    '''
    fun = fun_reshape
