...
def find_best_checkpoint(save_dir, net_type='G', epoch=None):
    """
    Args:
        save_dir: model folder
        net_type: 'G' or 'D' or 'optimizerG' or 'optimizerD'
    ...
    file_list = glob.glob(os.path.join(save_dir, 'best/*_{}.pth'.format(net_type)))
    if file_list:
        iter_exist = []
        for file_ in file_list:
            iter_current = re.findall(r"(\d+)_{}.pth".format(net_type), file_)
        ...
        init_iter = max(iter_exist)
        if epoch is None:
            init_path = os.path.join(save_dir, 'best/{}_{}.pth'.format(init_iter, net_type))
            return init_iter, init_path
        else:
            init_path = os.path.join(save_dir, 'best/{}_{}.pth'.format(epoch, net_type))
        return epoch, init_path
    else:
        init_iter = 0
        init_path = glob.glob(os.path.join(save_dir, 'best/*.pth'))[0]
...
def test_mode(model, L, mode=0, refield=32, min_size=256, sf=1, modulo=1):
    ...
    Args:
        ...
        mode:
            (0) normal: test(model, K)
            (1) pad: test_pad(model, K, modulo=16)
            (2) split: test_split(model, K, refield=32, min_size=256, sf=1, modulo=1)
            (3) x8: test_x8(model, K, modulo=1) ^_^
            (4) split and x8: test_split_x8(model, K, refield=32, min_size=256, sf=1, modulo=1)
...
def test_split_fn(model, L, refield=32, min_size=256, sf=1, modulo=1):
    ...
    if h * w <= min_size ** 2:
        L = torch.nn.ReplicationPad2d(
            (0, int(np.ceil(w / modulo) * modulo - w), 0, int(np.ceil(h / modulo) * modulo - h)))(L)
        E = model(L)
        E = E[..., :h * sf, :w * sf]
    else:
        ...
        if h * w <= 4 * (min_size ** 2):
            ...
            Es = [test_split_fn(model, Ls[i], refield=refield, min_size=min_size, sf=sf, modulo=modulo) for i in
                  range(4)]
...
def test_split_x8(model, L, refield=32, min_size=256, sf=1, modulo=1):
    E_list = [test_split_fn(model, util.augment_img_tensor4(L, mode=i), refield=refield, min_size=min_size, sf=sf,
                            modulo=modulo) for i in range(8)]
