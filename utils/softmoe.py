...
class DynamicSlotsSoftMoE(Module):
    ...
    def forward(self, x, mask = None):
        ...
        logits = einsum('b n d, b e K d -> b n e K', x, slot_embeds)

        # account for key padding mask

        if exists(mask):
            mask = rearrange(mask, 'b n -> b n 1 1')
            slots_mask = rearrange(slots_mask, 'b K -> b 1 1 K')
        ...
        combine_weights = rearrange(logits, 'b n e K -> b n (e K)')
        ...
        slots = einsum('b n d, b n e K -> e b K d', x, dispatch_weights)
        ...
        out = rearrange(out, 'e b K d -> b (e K) d')
        out = einsum('b K d, b n K -> b n d', out, combine_weights)
