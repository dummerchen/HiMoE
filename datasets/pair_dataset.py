...
class pairDataset(DeepfakeAbstractBaseDataset):
    ...
    def __getitem__(self, index, norm=True):
        ...
        if self.config.get('use_train_data_augmentation'):
            fake_image_trans, fake_landmarks_trans, fake_mask_trans = self.data_aug(fake_image, fake_landmarks,
                                                                                    fake_mask)
            real_image_trans, real_landmarks_trans, real_mask_trans = self.data_aug(real_image, real_landmarks,
                                                                                    real_mask)
        else:
            fake_image_trans, fake_landmarks_trans, fake_mask_trans = fake_image, fake_landmarks, fake_mask
            real_image_trans, real_landmarks_trans, real_mask_trans = real_image, real_landmarks, real_mask
        if not norm:
            return {
                "fake": (fake_image_trans, self.to_tensor(fake_image), fake_label),
                "real": (real_image_trans, self.to_tensor(real_image), real_label),
            }
        ...
        return {"fake": (fake_image_trans, fake_image, fake_label, fake_spe_label, fake_landmarks_trans,
                         fake_mask_trans, fake_image_path),
                "real": (real_image_trans, real_image, real_label, real_spe_label, real_landmarks_trans,
                         real_mask_trans, real_image_path),}
    ...
    def collate_fn(batch):
        ...
        fake_images, fake_raw_images, fake_labels, fake_spe_labels, fake_landmarks, fake_masks, fake_images_path = zip(
            *[data["fake"] for data in batch])
        real_images, real_raw_images, real_labels, real_spe_labels, real_landmarks, real_masks, real_images_path = zip(
        ...
        aug_image = torch.cat([real_images, fake_images], dim=0)
        raw_image = torch.cat([real_raw_images, fake_raw_images], dim=0)
        ...
        data_dict = {
            'aug_image': aug_image,
            'raw_image': raw_image,
            'spe_labels': spe_labels,
            'labels': labels,
            # 'landmarks': landmarks,
            'masks': masks,
            'path': real_images_path + fake_images_path,
        }
        return data_dict
