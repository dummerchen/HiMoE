...
from datasets.distortions import augmentation
...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def init_data_aug_method(self):
        trans = A.Compose([
            ...
            A.GaussNoise(var_limit=(
                eval(self.config['data_aug']['noise_limit_lower']), eval(self.config['data_aug']['noise_limit_upper'])),
                per_channel=True, p=0.5),
        ],
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        for label in dataset_info[dataset_name]:
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                if '\\' in frame_paths[0]:
                    frame_paths = sorted(frame_paths, key=lambda x: int(x.split('\\')[-1].split('.')[-2].splite('_')[-1]))
                else:
                    frame_paths = sorted(frame_paths, key=lambda x: int(x.split('/')[-1].split('.')[-2].split('_')[-1]))
        ...
        if not self.lmdb:
            ...
            with self.env.begin(write=False) as txn:
                ...
                for key, value in cursor:
                    path = os.path.join('/home/data3/deepfake', key.decode()).replace('\\', '/')
                    if 'DeepFake' in path or 'FaceShifter' in path:
                        continue
                    tot += 1
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        for image_path in image_paths:
            ...
            if self.mode == 'train' and self.config['use_train_data_augmentation']:
                image_trans, landmarks_trans, mask_trans = self.data_aug(image, landmark=landmarks, mask=mask)
            elif self.mode == 'test' and self.config['use_test_data_augmentation']:
                image_trans = augmentation(image, self.config['test_data_aug'], degradation_params=self.config['test_level'])
                landmarks_trans, mask_trans = deepcopy(landmarks), deepcopy(mask)
            else:
        ...
        return images, image_tensors, label, landmark_tensors, mask_tensors, image_paths[0]
    ...
    def collate_fn(batch):
        ...
        image, aug_image, labels, landmarks, masks, image_paths = zip(*batch)

        # Stack the image, label, landmark, and mask tensors
        image = torch.stack(image, dim=0)
        image_paths = list(image_paths)
        aug_image = torch.stack(aug_image, dim=0)
        ...
        data_dict = {}
        data_dict['raw_image'] = image
        data_dict['aug_image'] = aug_image
